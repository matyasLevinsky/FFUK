# Analyzing the CNN coverage of the Prague University shooting
...
I used the web-scraping tutorial provided by renatatopinkova and the book *Text Mining with R* by Julia Silge and David Robinson. ChatGPT-4 and its internal module R-Guru were used for troubleshooting.
...
library(janitor)
library(here)
source(here("CNNAnalysis/helperCNNAnalysis.R"))
```
...
parsedDt <- parseUrlList(htmlList)
saveRDS(parsedDt, file = "parsedDt_WebscrapingCNN")
parsedDt
```
...
```{r}
parsedDt <- readRDS(here("CNNAnalysis/parsedDB_WebscrapingCNN"))
parsedDt
```
...
## Expandin on that analysis: Latent Dirichlet allocation
